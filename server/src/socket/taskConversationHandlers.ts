...
const onlineUsers = new Map<number, Set<string>>(); // taskId -> Set of userIds
 ...
 */
export function setupTaskConversationHandlers(
  io: SocketIOServer,
   ...
   */
  socket.on('join-task-room', async (data: JoinTaskRoomData) => {
    try {
   ...
   */
  socket.on('leave-task-room', async (data: JoinTaskRoomData) => {
    try {
      const { taskId, companyId: taskCompanyId } = data;
  ...
  socket.on('send-message', async (data: SendMessageData) => {
    try {
      ...
      let conversation = await taskConversationService.getConversationByTaskId(
        taskId,
        companyId
      );
      if (!conversation) {
   ...
   * Mark user as online
   */
  socket.on('mark-online', async (data: TypingData) => {
    try {
      const { taskId, companyId: taskCompanyId } = data;
   ...
   */
  socket.on('mark-offline', async (data: TypingData) => {
    try {
      const { taskId, companyId: taskCompanyId } = data;
