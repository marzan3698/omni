...
export const paymentService = {
   ...
   */
  async getPaymentsByInvoice(invoiceId: number, companyId: number) {
   ...
   */
  async getPaymentsByClient(clientId: number, companyId: number) {
    return await prisma.payment.findMany({
      where: {
        clientId,
        companyId,
   ...
   */
  async getAllPayments(companyId: number, filters?: { status?: string; clientId?: number }) {
    return await prisma.payment.findMany({
      where: {
        companyId,
   ...
   * Approve payment
   */
  async approvePayment(paymentId: number, companyId: number, adminId: string, adminNotes?: string) {
   ...
   * Reject payment
   */
  async rejectPayment(paymentId: number, companyId: number, adminId: string, adminNotes?: string) {
    const payment = await prisma.payment.findFirst({
      where: {
        id: paymentId,
        companyId,
   ...
   * Get payment by ID
   */
  async getPaymentById(paymentId: number, companyId: number) {
    const payment = await prisma.payment.findFirst({
      where: {
        id: paymentId,
        companyId,
