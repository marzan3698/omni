...
const createLeadCallSchema = z.object({
  ...
  callTime: z.preprocess(
    (val) => (typeof val === 'string' || val instanceof Date ? new Date(val) : val),
    z.date({ required_error: 'Call time is required' })
  ),
...
export const leadCallController = {
   ...
   */
  getLeadCalls: async (req: Request, res: Response) => {
    try {
   ...
   */
  createLeadCall: async (req: Request, res: Response) => {
    try {
      ...
      const call = await leadCallService.createLeadCall({
        ...
        status: (validatedData.status as LeadCallStatus) || 'Scheduled',
      });
   ...
   */
  updateLeadCall: async (req: Request, res: Response) => {
    try {
      const leadId = parseInt(req.params.leadId);
      ...
      const call = await leadCallService.updateLeadCall(
        id,
        leadId,
        companyId,
        updateData
   ...
   */
  deleteLeadCall: async (req: Request, res: Response) => {
    try {
      const leadId = parseInt(req.params.leadId);
      const id = parseInt(req.params.id);
      const companyId = parseInt((req.query.companyId as string) || (req.body.companyId as string));
   ...
   */
  addCallNote: async (req: Request, res: Response) => {
    try {
      const leadId = parseInt(req.params.leadId);
      const id = parseInt(req.params.id);
      const companyId = parseInt((req.query.companyId as string) || (req.body.companyId as string));
  ...
  getAllCalls: async (req: Request, res: Response) => {
    try {
      ...
      if (req.query.status) {
        filters.status = req.query.status as LeadCallStatus;
      }
      ...
      const calls = await leadCallService.getAllCalls(
        user.companyId,
        user.id,
        user.role?.name,
   ...
   */
  getUpcomingCalls: async (req: Request, res: Response) => {
    try {
      const authReq = req as AuthRequest;
      const user = authReq.user;
