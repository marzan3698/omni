...
export const utilsController = {
   ...
   */
  getNgrokUrl: async (req: Request, res: Response) => {
    try {
   ...
   */
  subscribePage: async (req: Request, res: Response) => {
    try {
   ...
   */
  checkSubscription: async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const isSubscribed = subscriptions.some((sub: any) => sub.id === appId);
