...
export const companyController = {
   ...
   */
  getCompanyContacts: async (req: Request, res: Response) => {
    try {
   ...
   */
  addCompanyContact: async (req: Request, res: Response) => {
    try {
      const companyId = parseInt(req.params.id);
      if (isNaN(companyId)) {
        return sendError(res, 'Invalid company ID', 400);
   ...
   */
  getCompanyContracts: async (req: Request, res: Response) => {
    try {
      const companyId = parseInt(req.params.id);
      if (isNaN(companyId)) {
        return sendError(res, 'Invalid company ID', 400);
   ...
   */
  createCompanyContract: async (req: Request, res: Response) => {
    try {
      const companyId = parseInt(req.params.id);
      if (isNaN(companyId)) {
        return sendError(res, 'Invalid company ID', 400);
