...
import { TaskPriority, TaskStatus, AttachmentFileType, TaskMessageType } from '@prisma/client';
...
export const taskController = {
  ...
  getAllTasks: async (req: Request, res: Response) => {
    try {
      ...
      if (!isSuperAdmin) {
        companyId = user?.companyId || parseInt(req.query.companyId as string);
  ...
  getUserTasks: async (req: Request, res: Response) => {
    try {
      ...
      if (!isSuperAdmin && requestedUserId !== user?.id) {
        ...
        const permissions = userRecord?.role.permissions as Record<string, boolean>;
  ...
  getTaskDetail: async (req: Request, res: Response) => {
    try {
      ...
      const user = authReq.user;
      const id = parseInt(req.params.id);
      const companyId = user?.companyId || parseInt(req.query.companyId as string);
  ...
  uploadAttachment: async (req: Request, res: Response) => {
    try {
      ...
      let fileType: AttachmentFileType = 'image';
