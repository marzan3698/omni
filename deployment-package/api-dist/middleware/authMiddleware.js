 ...
 */
export const verifyTaskViewAccess = async (req, res, next) => {
    try {
        const authReq = req;
        if (!authReq.user) {
            return res.status(401).json({
 ...
 */
export const verifyTaskUpdateAccess = async (req, res, next) => {
    try {
        const authReq = req;
        if (!authReq.user) {
            return res.status(401).json({
        ...
        if (!isNaN(taskId)) {
            ...
            if (task.assignedTo === user.employee.id) {
                ...
                const isOnlyStatusUpdate = Object.keys(updateData).every(key => allowedFields.includes(key));
 ...
 */
export const verifyLeadAccess = async (req, res, next) => {
    try {
        const authReq = req;
        if (!authReq.user) {
            return res.status(401).json({
