 ...
 */
export var WebhookMode;
(function (WebhookMode) {
    WebhookMode["local"] = "local";
    WebhookMode["live"] = "live";
 ...
 */
export async function getChatwootWebhookUrl(mode = WebhookMode.local, integrationId) {
 ...
 * @returns Webhook URL string
 */
export async function getIntegrationWebhookUrl(integrationId) {
    try {
        const { prisma } = await import('../lib/prisma.js');
        const integration = await prisma.integration.findUnique({
            where: { id: integrationId },
        ...
        if (integration.provider === 'chatwoot') {
            return await getChatwootWebhookUrl(integration.webhookMode || WebhookMode.local, integrationId);
        }
