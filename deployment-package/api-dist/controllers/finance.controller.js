...
export const financeController = {
    ...
    getAllInvoices: async (req, res) => {
        try {
            ...
            if (req.query.clientId)
                filters.clientId = parseInt(req.query.clientId);
    ...
    getInvoiceById: async (req, res) => {
        try {
            ...
            const companyId = req.user?.companyId || parseInt(req.query.companyId || req.body.companyId);
            ...
            if (req.user?.role?.name === 'Client') {
                ...
                const clientEmail = invoice.client?.contactInfo && typeof invoice.client.contactInfo === 'object'
                    ? invoice.client.contactInfo.email?.toLowerCase()
                    : null;
    ...
    getAllReceivables: async (req, res) => {
        try {
            const companyId = parseInt(req.query.companyId);
            if (isNaN(companyId)) {
                return sendError(res, 'Company ID is required', 400);
    ...
    createReceivable: async (req, res) => {
        try {
