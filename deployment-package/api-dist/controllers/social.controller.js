...
export const socialController = {
     ...
     */
    verifyFacebookWebhook: (req, res, next) => {
        try {
     ...
     */
    handleFacebookWebhook: async (req, res, next) => {
        try {
     ...
     */
    getConversationAnalytics: async (req, res, next) => {
        try {
            const companyId = req.user?.companyId;
     ...
     */
    getPublicConversationAnalytics: async (req, res, next) => {
        try {
    ...
    sendReply: async (req, res, next) => {
        try {
            ...
            const message = await socialService.sendReply(conversationId, content.trim(), agentId, imageUrl);
     ...
     */
    updateTypingStatus: async (req, res, next) => {
        try {
            const conversationId = parseInt(req.params.id, 10);
            const userId = req.user?.id;
     ...
     */
    getTypingStatus: async (req, res, next) => {
        try {
            const conversationId = parseInt(req.params.id, 10);
            if (isNaN(conversationId)) {
                return sendError(res, 'Invalid conversation ID', 400);
     ...
     */
    assignConversation: async (req, res, next) => {
        try {
            const conversationId = parseInt(req.params.id, 10);
            const companyId = req.user?.companyId;
            const userId = req.user?.id;
            ...
            const updatedConversation = await socialService.assignConversation(conversationId, employee.id, companyId);
     ...
     */
    unassignConversation: async (req, res, next) => {
        try {
            const conversationId = parseInt(req.params.id, 10);
            const companyId = req.user?.companyId;
            const userId = req.user?.id;
     ...
     */
    getConversationStats: async (req, res, next) => {
        try {
            const companyId = req.user?.companyId;
            if (!companyId) {
                return sendError(res, 'User not authenticated', 401);
     ...
     */
    getConversationReleaseHistory: async (req, res, next) => {
        try {
            const conversationId = parseInt(req.params.id, 10);
            const companyId = req.user?.companyId;
            if (isNaN(conversationId)) {
