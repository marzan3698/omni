...
export const subTaskService = {
     ...
     * Update sub-task
     */
    async updateSubTask(id, companyId, data) {
     ...
     * Delete sub-task
     */
    async deleteSubTask(id, companyId) {
        const subTask = await prisma.subTask.findFirst({
            where: {
                id,
                companyId,
     ...
     */
    async getSubTasksByTaskId(taskId, companyId) {
        // Verify task exists and belongs to company
        const task = await prisma.task.findFirst({
            where: {
                id: taskId,
     ...
     */
    async calculateTaskProgress(taskId, companyId) {
        const subTasks = await prisma.subTask.findMany({
            where: {
                taskId,
                companyId,
