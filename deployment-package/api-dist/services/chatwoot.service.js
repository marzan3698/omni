 ...
 */
function getChatwootApiUrl(baseUrl) {
 ...
 */
function getChatwootHeaders(accessToken) {
    return {
...
export const chatwootService = {
     ...
     */
    async getChatwootConversations(accountId, inboxId, accessToken, baseUrl) {
        try {
     ...
     */
    async getChatwootMessages(accountId, conversationId, accessToken, baseUrl) {
        try {
            const apiUrl = getChatwootApiUrl(baseUrl);
     ...
     */
    async sendChatwootMessage(accountId, conversationId, content, accessToken, baseUrl, attachments) {
        try {
            const apiUrl = getChatwootApiUrl(baseUrl);
            const url = `${apiUrl}/api/v1/accounts/${accountId}/conversations/${conversationId}/messages`;
    ...
    async syncChatwootConversations(accountId, inboxId, accessToken, baseUrl) {
        try {
            ...
            const chatwootConversations = await this.getChatwootConversations(accountId, inboxId, accessToken, baseUrl);
            let syncedCount = 0;
            let createdCount = 0;
            let updatedCount = 0;
            for (const chatwootConv of chatwootConversations) {
                ...
                try {
                    const messages = await this.getChatwootMessages(accountId, chatwootConv.id, accessToken, baseUrl);
     ...
     */
    async processChatwootWebhook(payload) {
        ...
        try {
            ...
            if (attachments && Array.isArray(attachments) && attachments.length > 0) {
                ...
                const imageAttachment = attachments.find((att) => att.type === 'image' || att.file_type === 'image' || att.content_type?.startsWith('image/'));
                if (imageAttachment) {
                    try {
