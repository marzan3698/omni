...
export const audioService = {
    ...
    async uploadAudio(data) {
        ...
        const uploadDir = path.join(process.cwd(), 'uploads', 'tasks', data.taskId ? `task-${data.taskId}` : `subtask-${data.subTaskId}`, 'audio');
        ...
        try {
            ...
            try {
                ...
                // Convert to MP3
                await execAsync(`ffmpeg -i "${originalFilePath}" -codec:a libmp3lame -b:a 128k "${mp3FilePath}" -y`);
