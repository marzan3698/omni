...
export const socialService = {
     ...
     */
    verifyWebhook(verifyToken, challenge, mode) {
     ...
     */
    async getConversations(status, companyId, tab, assignedToEmployeeId) {
        const where = {};
        if (status) {
            where.status = status;
        }
     ...
     */
    async getConversationAnalytics(companyId, days = 30) {
        ...
        const messageDailyMap = messageGroups.reduce((acc, item) => {
            const key = formatDateKey(item.createdAt);
            acc[key] = (acc[key] || 0) + item._count._all;
            return acc;
        }, {});
        const conversationDailyMap = conversationGroups.reduce((acc, item) => {
            const key = formatDateKey(item.createdAt);
            acc[key] = (acc[key] || 0) + item._count._all;
            return acc;
        }, {});
        ...
        const platformBreakdown = {
            ...
            other: platformCounts.reduce((sum, p) => {
                if (p.platform === 'facebook' || p.platform === 'chatwoot')
                    return sum;
     ...
     */
    async getConversationMessages(conversationId, companyId) {
     ...
     */
    async sendReply(conversationId, content, agentId, imageUrl) {
        ...
        if (conversation.platform === 'chatwoot') {
            ...
            if (conversationIdMatch) {
                ...
                if (contactIdMatch) {
                    ...
                    try {
                        const chatwootConversations = await chatwootService.getChatwootConversations(integration.accountId, inboxId, integration.accessToken, integration.baseUrl);
                        // Find conversation for this contact
                        console.log(`Searching ${chatwootConversations.length} conversations for contact ${contactId}`);
                        const chatwootConv = chatwootConversations.find(conv => conv.contact.id.toString() === contactId);
                        if (chatwootConv) {
                            ...
                            console.log(`❌ Conversation not found for contact ${contactId}. Available contacts:`, chatwootConversations.map(c => `${c.contact.id} (${c.contact.name})`).join(', '));
                            ...
                            try {
                                const axios = (await import('axios')).default;
                                const getChatwootApiUrl = (baseUrl) => {
            ...
            try {
                ...
                chatwootMessage = await chatwootService.sendChatwootMessage(integration.accountId, chatwootConversationId, content || '', // Chatwoot requires content, even if empty
                integration.accessToken, integration.baseUrl, attachments // Pass attachments
                );
                ...
                throw new AppError(`Failed to send message to Chatwoot: ${error.response?.data?.error || error.message}`, error.response?.status || error.statusCode || 500);
            }
        ...
        if (conversation.platform === 'facebook') {
            ...
            try {
                ...
                throw new AppError(`Failed to send message to Facebook: ${error.response?.data?.error?.message || error.message}`, error.response?.status || 500);
            }
        }
        else {
    ...
    async syncChatwootConversations() {
        ...
        for (const integration of integrations) {
            ...
            try {
                const result = await chatwootService.syncChatwootConversations(integration.accountId, integration.pageId, // inboxId
                integration.accessToken, integration.baseUrl);
                results.push({
                    integrationId: integration.id,
                    ...result,
                });
     ...
     */
    async markMessagesAsRead(conversationId, userId, companyId) {
     ...
     */
    async markMessageAsRead(messageId, userId, companyId) {
     ...
     */
    async updateSeenStatus(conversationId, messageIds, companyId) {
        // Verify conversation exists and belongs to company
        const conversation = await prisma.socialConversation.findFirst({
            where: {
                id: conversationId,
     ...
     */
    async assignConversation(conversationId, employeeId, companyId) {
        // Verify conversation exists and belongs to company
        const conversation = await prisma.socialConversation.findFirst({
            where: {
                id: conversationId,
     ...
     */
    async unassignConversation(conversationId, companyId, employeeId) {
        // Verify conversation exists and belongs to company
        const conversation = await prisma.socialConversation.findFirst({
            where: {
                id: conversationId,
     ...
     */
    async completeConversation(conversationId, companyId) {
        // Verify conversation exists and belongs to company
        const conversation = await prisma.socialConversation.findFirst({
            where: {
                id: conversationId,
     ...
     */
    async getConversationReleaseHistory(conversationId, companyId) {
        // Verify conversation exists and belongs to company
        const conversation = await prisma.socialConversation.findFirst({
            where: {
                id: conversationId,
